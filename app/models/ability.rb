...
class Ability
  ...
  def initialize(user)
    ...
    can :update_conference_preferences, Team do |team|
      team.accepted? && team.students.include?(user) && signed_in?(user)
    end

    can :see_offered_conferences, Team do |team|
      (user.admin? || team.students.include?(user) || team.supervisors.include?(user)) && signed_in?(user)
    end

    can :accept_or_reject_conference_offer, Team do |team|
      team.students.include?(user) && signed_in?(user)
    end
    ...
    can :crud, Role do |role|
      (user.admin? || on_team?(user, role.team)) && signed_in?(user)
    end
    ...
    can :supervise, Team do |team|
      (user.roles.organizer.any? || team.supervisors.include?(user)) && signed_in?(user)
    end

    can :crud, ConferencePreference do |preference|
      (user.admin? || (preference.team.students.include? user)) && signed_in?(user)
    end

    can :crud, Conference if (user.admin? || user.current_student?) && signed_in?(user)
    ...
    can :crud, :comments if (user.admin?) && signed_in?(user)
    can :read, :users_info if (user.admin? || user.supervisor?) && signed_in?(user)

    # projects
    can :crud, Project do |project|
      (user.admin? ||
       (user.confirmed? && user == project.submitter)) && signed_in?(user)
    end
    can :use_as_template, Project do |project|
      (user == project.submitter && !project.season&.current?) && signed_in?(user)
    end
    ...
    # applications
    can :create, :application_draft if user.student? && user.application_drafts.in_current_season.none? && signed_in?(user)
  end
